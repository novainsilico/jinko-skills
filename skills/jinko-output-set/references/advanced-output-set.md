...
- `filter`: optional condition restricting which time points or arms it applies to. Time values are seconds; convert scientific thresholds explicitly. Do not use ISO durations such as `"time > PT47H"`.
...
- `unit`: optional unit string accepted on scalars, e.g. `"mg/L*s"`. It declares the scalar's expected unit; it is distinct from the unsupported `u(...)` expression syntax. Time-reduction functions use seconds.
- `description`, `display_name`, `is_active`: optional metadata.
...
- **Objective `target` referencing a sibling scalar's `id` can fail `trial.sanity()`.** Observed on a live trial: `client.create_advanced_output_set(scalars=[{"id": "tumor_burden_auc", "formula": "auc(T)", ...}], objectives=[{"id": "obj_x", "formula": {"target": "tumor_burden_auc", ...}}])` passes `scoring_design.diagnostics` cleanly, but attaching to a trial and calling `trial.sanity()` reports `ADVANCED_OUTPUTS_ERRORS` for the objective only. Put the scalar's formula directly in the objective target (`"target": "auc(T)"`) instead of referencing the scalar id.
...
scoring_design.components.add_scalar("auc", formula="auc(Drug)", unit="mg/L*s")
scoring_design.components.add_objective(
    "obj_auc",
    formula_target="auc(Drug)",
