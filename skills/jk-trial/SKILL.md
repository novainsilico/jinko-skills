...
name: jk-trial
description: >-
  Create, sanity-check, run, poll, and download results for Jinkō in-silico trials via the jinko-sdk. Use this skill whenever the user wants to set up a trial from a computational model and simple output set, optionally attach a vpop, protocol, data table, or advanced scoring output set, launch a trial, wait for completion, inspect completed trials, or download TimeSeries and Scalar results as pandas DataFrames. Do not use this skill for model editing, vpop creation, protocol design authoring, data-table upload, or trial visualization.
compatibility: >-
...
Use this skill for trial setup, sanity checks, run/poll, and result download. Keep creation of upstream assets in their dedicated skills: `jk-model`, `jk-vpop`, `jk-protocol`, and `jk-data-table`.
...
- Create simple output set: `client.create_simple_output_set(model, model.time_dependent_ids())` unless explicit output ids were requested.
- Create trial: `client.create_trial(model, vpop=..., protocol=..., output_set=..., scoring=...)`.
- Run trial: `trial.run()`.
