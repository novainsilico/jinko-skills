...
name: jk-model
description: >-
  Build or edit a Jinkō computational model (QSP/PK-PD) via the jinko-sdk: parameters, categorical parameters, compartments, species, ODEs, reactions, dosing events, algebraic or assignment rules, baseline checks, and solving options. Use this skill whenever the user wants to create a model from scratch, create an empty model, edit an existing model, add or modify components, configure unit checking, define model-level dosing events, validate diagnostics, or debug model sanity or simple_solve errors. Prefer editing existing models over recreating them. Do not use this skill for running trials; use jk-trial for trial execution.
compatibility: >-
  Check set-up with the `jk-sdk-setup` skill. Model creation/editing requires write access to the Jinkō project.
metadata:
...
```bash
python skills/jk-model/scripts/create_minimal_model.py --name sdk-minimal-model
python skills/jk-model/scripts/create_minimal_model.py --name sdk-minimal-model --apply
python skills/jk-model/scripts/create_minimal_model.py --name sdk-minimal-model --folder 2026-06-15-pk-study --create-folder --apply
python skills/jk-model/scripts/edit_existing_model_batch.py --model-sid cm-... --apply
python skills/jk-model/scripts/validate_model_readiness.py --model-sid cm-...
python skills/jk-model/scripts/validate_model_readiness.py --model-sid cm-... --timeseries-id Drug
```
...
Use `create_algebraic_rule()` or `edit_algebraic_rule()` for implicit constraints and cyclic dependencies. Jinkō algebraic rules are residual equations: the `equation` value is interpreted as `0 = equation`.

Convert explicit implicit definitions into residual form:

- `x1 = f1(x1, x2)` becomes `f1(x1, x2) - x1`.
- `x2 = f2(x1, x2)` becomes `f2(x1, x2) - x2`.

Add one algebraic rule per residual equation. If diagnostics report `TOO_FEW_ALGEBRAIC_RULES`, `TOO_MANY_ALGEBRAIC_RULES`, or `MISSING_UNKNOWN`, check that the number of algebraic equations matches the number of unknown algebraic variables.
...
- Read `references/model-components.md` for component batching examples and method names.
