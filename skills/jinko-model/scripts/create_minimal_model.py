...
def report_simple_solve(model: Any, timeseries_id: str) -> bool:
    ...
    if result.error:
        ...
        return True
    series = next((item for item in result.results if item.id == timeseries_id), None)
    if series is None:
        print(
            f"simple_solve did not return requested event-affected series: {timeseries_id}",
            file=sys.stderr,
        )
        return True
    if len(series.values) < 2 or all(
        ...
        print(
            f"Expected event-driven change was not observed in {timeseries_id}: "
            f"{series.values}",
            file=sys.stderr,
        )
        return True
