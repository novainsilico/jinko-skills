...
Field-tag cheatsheet for queries built by `jinko-task-literature-search` and its intent recipes. Bare keyword queries are a last resort; every angled query should use at least one tag from this table.
...
| `Lastname FN[au]` | **Author** tag. Use in a user-approved follow-up after the first pass surfaces relevant authors. Initial format is `LastName FN` (first initial only). |
...
- **Between clauses**, use `AND` to compose entity, publication-type, date, and species constraints. Run distinct angles separately so provenance is preserved.
...
- **`[au]` and `[ta]` usually follow discovery.** Use them after the first pass surfaces relevant authors and journals, unless authoritative seeds were supplied.
