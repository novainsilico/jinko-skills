...
def _reference_from_match(
    *,
    ...
    reference["citation_ama"] = _ama_citation(reference, crossref_item)
    return reference
    ...
    *,
    ...
    for reference in references:
        text = " ".join([
            ...
            str(reference.get("citation_ama", "")),
        ])
...
def run_literature_search(
    *,
    ...
    # Flag seeded references so downstream compilation can identify anchors.
    if seed_pmid_set:
        for ref in references:
    ...
    (output_dir_abs / "references_ama.txt").write_text(
        "\n".join(
            item.get("citation_ama", "")
            for item in references
            if item.get("citation_ama")
        )
        + ("\n" if references else ""),
        encoding="utf-8",
    )
    ...
    manifest = {
        "stage": "jinko-task-literature-search-standalone",
        "query": query,
        "retmax": safe_retmax,
        "sort": safe_sort,
